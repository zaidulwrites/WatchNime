// admin-frontend/src/pages/AddEditAnime.tsx
...
interface AddAnimeFormData {
  title: string;
  description: string;
  poster: string; // Now only one poster URL
  allDetails: string;
  genreNames: string[];
}
...
const AddEditAnime = () => {
    ...
    if (id) {
      ...
      if (existingAnime) {
        setFormData({
          ...
          genreNames: existingAnime.genres ? existingAnime.genres.map(g => g.name) : [],
        });
      } else {
        const loadAnime = async () => {
          ...
          try {
            ...
            setFormData({
              ...
              genreNames: fetchedAnime.genres ? fetchedAnime.genres.map(g => g.name) : [],
            });
          } catch (err) {
  ...
  const handleGenreChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setFormData((prevData) => ({ ...prevData, genreNames: selectedOptions }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && id) {
        await adminApi.updateAnimeAdmin(id, formData, formData.genreNames);
        alert('Anime updated successfully!'); // Use custom modal in production
      } else {
        await adminApi.addAnimeAdmin(formData, formData.genreNames);
        alert('Anime added successfully!'); // Use custom modal in production
        setFormData({ // Clear form after adding
          title: '', description: '', poster: '', allDetails: '', genreNames: []
        });
      }
